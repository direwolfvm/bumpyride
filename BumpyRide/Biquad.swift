...
struct Biquad {
    ...
    /// Constant-skirt-gain bandpass coefficients via the RBJ Audio EQ Cookbook.
    /// The −3 dB points sit roughly at `centerHz · 2^(±1/(2Q))`; with `q = 1` and
    /// `centerHz = 2`, that's about [1.4 Hz, 2.8 Hz] — adequate to capture the
    /// 60–110 RPM cadence fundamental (1.0–1.83 Hz) plus a little slack.
    static func butterworthBandPass(centerHz: Double, q: Double, sampleRateHz: Double) -> Biquad {
        let omega = 2.0 * .pi * centerHz / sampleRateHz
        let cosOmega = cos(omega)
        let alpha = sin(omega) / (2.0 * q)
