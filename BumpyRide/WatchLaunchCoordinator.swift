...
final class WatchLaunchCoordinator {
    ...
    func considerLaunchingWatchApp() async {
        ...
        // land here.
        guard settings.openWatchAppOnLaunch else {
        ...
        // sessionWatchStateDidChange delegate callback.
        guard watchCoordinator.isPaired else {
            state = .skipped(reason: "no paired Apple Watch")
            return
        }
        guard watchCoordinator.isWatchAppInstalled else {
        ...
        guard healthKitAuth.canWrite else {
        ...
        do {
            ...
            state = .launched
            Self.log.info("startWatchApp succeeded")
        } catch {
            let message = String(describing: error)
            Self.log.error("startWatchApp failed: \(message, privacy: .public)")
